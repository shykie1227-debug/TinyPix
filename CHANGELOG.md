...
### 2026-07-22 Windows 10 WinUI 可行性门禁收口

- 在真实 Windows 10 22H2 x64 VM 完成可丢弃 WinUI 原型门禁：自包含启动、100/125/150% 显示缩放、200% 文本缩放、主题、设置弹窗、键盘焦点、Narrator、拖放、图片/视频预览、FFmpeg 缩略图回退、取消/异常退出、断网和只读目录场景均有报告与截图证据。
- 使用 Process Monitor 复核注册表行为：12 个原始候选均未形成 TinyPix 自有写入，系统 BAM 执行状态记录单独分类；临时门禁计划任务已清理，原始 PML/CSV 保留在本地并忽略提交。
- 明确门禁原型仅用于运行可行性验证，不作为正式 UI 或业务实现来源；正式 WinUI Shell 仍需 Windows 11 干净 VM 验证后，从 Pencil 设计源和原生控件重新实现。
- 修复 SQLite 历史仓库连接池导致 Windows 数据库文件句柄延迟释放的问题，并增加“操作完成后可删除数据库文件”的回归测试。

### 2026-07-19 Windows SQLite 文件句柄释放

- Windows 10 VM 首次运行 Infrastructure 测试时复现 `tinypix.db` 在操作完成后仍被占用；新增显式回归测试，要求仓库方法返回后数据库文件可删除。
- 根因是 `Microsoft.Data.Sqlite` 默认连接池在 Dispose 后继续持有数据库句柄；历史仓库连接串改为 `Pooling=false`，确保每次短连接离开 `await using` 后真实释放文件，不用重试或全局清池掩盖问题。
