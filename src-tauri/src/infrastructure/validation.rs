...
pub fn validate_time_range(start_secs: f64, end_secs: f64, total_secs: f64) -> Result<(), TinyPixError> {
    if start_secs < 0.0 {
        return Err(TinyPixError::InvalidParam("起始时间不能为负数".to_string()));
    }
    if start_secs >= end_secs {
        return Err(TinyPixError::InvalidParam("起始时间必须小于结束时间".to_string()));
    }
...
mod tests {
    ...
    fn test_validate_video_path_not_exists() {
        ...
        assert!(matches!(result, Err(TinyPixError::InvalidParam(ref s)) if s.contains("文件不存在")));
    }
    ...
    fn test_validate_output_path_empty() {
        ...
        assert!(matches!(result, Err(TinyPixError::InvalidParam(ref s)) if s == "输出路径不能为空"));
    }

    #[test]
    fn test_validate_output_path_parent_not_exists() {
        let result = validate_output_path("/nonexistent_dir/output.mp4");
        assert!(matches!(result, Err(TinyPixError::InvalidParam(ref s)) if s.contains("输出目录不存在")));
    }
    ...
    fn test_validate_time_range_negative_start() {
        ...
        assert!(matches!(result, Err(TinyPixError::InvalidParam(ref s)) if s == "起始时间不能为负数"));
    }

    #[test]
    fn test_validate_time_range_start_equals_end() {
        let result = validate_time_range(5.0, 5.0, 100.0);
        assert!(matches!(result, Err(TinyPixError::InvalidParam(ref s)) if s == "起始时间必须小于结束时间"));
    }
    ...
    fn test_validate_time_range_end_exceeds_total() {
        ...
        assert!(matches!(result, Err(TinyPixError::InvalidParam(ref s)) if s.contains("超过视频总时长")));
    }
